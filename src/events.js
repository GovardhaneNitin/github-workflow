...
function rollEvent(state, rng) {
  const roll = rng.next();
  if (roll < 0.10) return { type: 'treasure', loot: '💎', effect: { hp: +1 } };
  if (roll < 0.18) return { type: 'enemy', enemy: '👾', damage: 1 + Math.floor(rng.next()*3) };
...
function applyEvent(state, event) {
  if (event.type === 'treasure') {
    state.hero.hp = Math.min(state.hero.maxHp, state.hero.hp + (event.effect.hp||0));
